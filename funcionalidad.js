    ...
    class Calculator {
        ...
        async confirm() {
            ...
            if (!this.dividingMode) {
                ...
                //crea el pie-pag de la calcu
                const pie_pagina = document.getElementById("calculator");
                let foot_calcu = pie_pagina.querySelector(".foot-calcu");

                if (!foot_calcu) {
                    foot_calcu = document.createElement("div");
                    foot_calcu.classList.add("foot-calcu");

                    foot_calcu.innerHTML =
                        `
                        <p>Cantidad restante <strong class="total-propinas-strong"></strong></p>
                    `;

                    Object.assign(foot_calcu.style, {
                        borderRadius: "20px",
                        background: "#ffffff",
                        border: "2px solid #f89b8f",
                        margin: "15px 0",
                        padding: "0",
                        fontSize: "14px",
                        width: "100%",
                ...
                //actualiza monto de CANTIDAD RESTANTE
                const strongElement = document.querySelector(".total-propinas-strong");
                ...
                if (!isNaN(numPersonas) && numPersonas > 0) {
                    ...
                    this.displayElement.textContent = `$${propinaPorPersona.toFixed(2)}`;
                }
                ...
                //
                const strongElement = document.querySelector("#calculator .total-propinas-strong");
                if (strongElement) {
                    const formattedTotal = this.formatNumber(this.totalPropina.toString());
                    strongElement.textContent = `$${formattedTotal}`;
        ...
        async editInput() { //icono edita total propinas
            const inputPropina = document.querySelector(".input-propina"); //total propinas
            ...
            const numPersonas = parseInt(divInput.value); //actualiza valores (lado izquierdo)
            const divText = document.querySelector(".dividir-prop p");
            if (isNaN(numPersonas) || numPersonas <= 0) {
                divText.textContent = '$0.00 x persona';
            }

            //limpia el strong si ya hay total de propinas y se va editar
            const strongElement = document.querySelector("#calculator .total-propinas-strong");
            if (strongElement) {
                strongElement.textContent = '$0.00';
            }
